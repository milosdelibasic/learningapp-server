...
class AuthController {
  async processCommand(req, res, next) {
    ...
    try {
      ...
      switch (mode) {
        case "register":
          result = await service.registerUser(req, data);
          break;
        case "login":
          result = await service.login(req, data);
          break;
        case SocialMode.NEW:
      ...
      logger.info(
        req,
        `Successfully finished ${data.social_type} ${mode} action on auth data `
      );
      req.response = result;
      next();
    } catch (error) {
      logger.error(
        req,
        `Error processing ${data.social_type} ${mode} action on auth data `,
        error
      );
      next(error);
    }
